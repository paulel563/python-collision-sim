...
# Particle settings
PARTICLE_RADIUS = 1  # Initial radius for all particles
PARTICLE_COLOR = (0, 0, 0)  # Uniform color (green) for all particles
PARTICLE_SPEED = 1.5  # Uniform speed for all particles
NUM_PARTICLES = 2000  # Number of particles in the simulation
PARTICLE_GROWTH = 1  # Amount by which particles grow or shrink on collision
MAX_PARTICLE_RADIUS = 14  # Maximum allowed radius for particles
MIN_PARTICLE_RADIUS = 2  # Minimum allowed radius for particles
...
# Growth mode
growth_mode = 'growing'  # Controls whether particles are growing or shrinking

# Initial pause time
INITIAL_PAUSE_SECONDS = 2  # Time to pause at the beginning before starting
...
pygame.display.set_caption("Optimized Growth and Shrink Simulation")
...
class Particle:
    ...
    def adjust_color(self):
        ...
        min_radius = MIN_PARTICLE_RADIUS
    ...
    def resolve_collision(self, other):
        ...
        elif growth_mode == 'shrinking':
            if self.radius > MIN_PARTICLE_RADIUS:
                self.radius -= PARTICLE_GROWTH
            if other.radius > MIN_PARTICLE_RADIUS:
                other.radius -= PARTICLE_GROWTH
...
def check_growth_state(particles):
    """Checks if all particles are at their maximum or minimum size and toggles growth mode."""
    global growth_mode

    if growth_mode == 'growing' and all(p.radius >= MAX_PARTICLE_RADIUS for p in particles):
        growth_mode = 'shrinking'
    elif growth_mode == 'shrinking' and all(p.radius <= MIN_PARTICLE_RADIUS for p in particles):
        growth_mode = 'growing'
...
def main():
    ...
    while pygame.time.get_ticks() - start_time < INITIAL_PAUSE_SECONDS * 1000:
