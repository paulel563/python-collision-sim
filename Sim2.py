...
LAST_NUM_PARTICLES = 170  # Threshold for a forced color dominance swap

MIDDLE_LAST_NUM_PARTICLES = 300
MIDDLE_GROUP = 9

SECOND_LAST_NUM_PARTICLES = 60
...
COLOR2 = (0, 0, 220)  # "Blue"
COLOR1 = (0, 255, 127)# "Spring Green"
COLOR2_NAME = "Blue"
COLOR1_NAME = "Spring Green"
...
generate_ambient_progression("ambient.wav", AMBIENT_DURATION, 0.4, 44100)
...
try:
    ...
    ambient_sound = collision_sound = victory_sound = None
...
# -------------------------------------------------------
def generate_ambient_chord_swap_sound(chord, duration=0.8, volume=0.2, sample_rate=44100):
    """
    ...
    # If the mixer is mono, make a 1D array; if stereo, make a 2D array, etc.
    if channels == 1:
        data = data_mono  # shape: (samples,)
    else:
...
def check_last_particles(particles, elapsed_time):
    ...
    # if we swapped dominance, generate swap sound
    if previous_dominant != dominant_color:
        ...
        if current_tick - last_swap_sound_tick > SWAP_SOUND_COOLDOWN_MS:
            ...
            swap_sound = generate_ambient_chord_swap_sound(current_chord, 0.8, 0.2, 44100)
            swap_sound.play()
...
def main():
    ...
    while running:
        ...
        if SHOW_SCOREBOARD:
            ...
            right_pos = (SCREEN_WIDTH - right_text_w - 18, 18)
